...
class RasterPlotter:
    ...
    def plot_raster_bands(self, raster_file, band_names=None, band=None):
        ...
        Parameters:
        - raster_file (str): band raster path.
        - band_names (list, optional): Optional band names. 
        - band (int, optional): Plot specific band, or all bands (1-indexed)
        Returns:
        - None
        """
        try:
...
def plot_raster_single(raster_file, cmap='viridis', tick_labels=None):
        """
        Plot a single raster image with a colormap.
        """
        try:
            with rasterio.open(raster_file) as src:
                ...
                cbar = plt.colorbar(orientation='vertical',
                                    shrink=0.35, fraction=0.15, pad=0.01)  # Modify shrink and fraction for thickness
                cbar.set_ticks([vmin, vmax])
                cbar.set_label('', fontsize=18)  # Increase colorbar label font size
